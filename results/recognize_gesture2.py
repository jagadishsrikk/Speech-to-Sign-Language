...
def get_pred_text_from_db(pred_class):
	...
	for row in cursor:
		...
		if temp.lower().strip()=="fuck":
			...
			return "M"
		elif temp.lower().strip()=="best of luck":
			return "_"
		elif temp=="1":
			return "D"
		elif temp=="2":
			return "V"
		elif temp.lower().strip()=="i/me":
			return "G"
		elif temp.lower().strip()=="i love you":
			return "L"
		elif temp.lower().strip()=="8":
			return "A"
		return row[0]
...
def recognize():
	...
	while True:
		...
		grey = cv2.cvtColor(imgCrop,cv2.COLOR_BGR2GRAY)
		blurred = cv2.GaussianBlur(grey,val,0)

		(_,thresh)=cv2.threshold(blurred,lower_thresh,upper_thresh,cv2.THRESH_BINARY_INV+cv2.THRESH_OTSU)
